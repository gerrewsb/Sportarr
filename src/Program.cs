...
using Sportarr.Extensions;
using Sportarr.Services;
using System.IO.Abstractions;
using Sportarr.Endpoints;
#if WINDOWS
...
//Add services
builder.Services.AddSportarrServices();

//Add hosted services
builder.Services.AddSportarrHostedServices();

//Add DbContext
builder.Services.AddSportarrDbContext(dataPath);

//Add TestableIO IFilesystem so we can abstract the FileSytem for better testability
builder.Services.AddSingleton<IFileSystem, FileSystem>();

// Add memory cache for download client caching with expiration
builder.Services.AddMemoryCache();

// Add MVC controllers for AuthenticationController
...
try
{
    ...
    await dbService.UpdateStandardFileFormat();
    await dbService.SyncSecuritySettings();
...
var serverAgentsService = scope.ServiceProvider.GetRequiredService<ServerAgentsService>();
await serverAgentsService.CopyMediaServerAgents(dataPath);

// Configure middleware pipeline
await app.ConfigureUrlBase();
...
app.ConfigureStaticFilesMiddleWare();

//map endpoints
app.MapUtilityEndpoints()
    .MapAuthenticationEndpoints()
    .MapSystemEndpoints(dataPath)
    .MapStatsEndpoints()
    .MapLibraryEndpoints()
    .MapEventEndpoints()
    .MapLogEndpoints(logsPath)
    .MapTaskEndpoints()
    .MapEventsEndpoints()
    .MapLeaguesEndpoints()
    .MapTagEndpoints()
    .MapQualityProfileEndpoints()
    .MapCustomFormatEndpoints()
    .MapTrashGuidesEndpoints()
    .MapDelayProfileEndpoints();
